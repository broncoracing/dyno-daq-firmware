...
public:
  ...
  Hx711(PinName pin_sck, PinName pin_dt, int offset, float scale,
        uint8_t gain = 128)
      : sck_(pin_sck), dt_(pin_dt) {
    set_offset(offset);
    set_scale(scale);
    set_gain(gain);
  ...
  /**
   Set multiplier for A
   */
  void setMultiplierA(float arg_) { multA_ = arg_; }

  /**
   * set multiplier for B
   */
  void setMultiplierB(float arg_) { multB_ = arg_; }

  /**
   * manually set tare values
   */
  void setTareValA(float arg_) { tareValA_ = arg_; }

  void setTareValB(float arg_) { tareValB_ = arg_; }

  /**
   * auto tare the scales
   */
  float tareA(int times, double seconds);
  float tareB(int times, double seconds);
   ...
   * becomes ready...
   */
  bool is_ready() { return dt_.read() == LOW; }
   ...
   * @return float
   */
  float read() { return abs(convert_to_real(readRaw())); }

  // reads the two gained scales (32 or 64)
  float readA();
  float readB();

  // reads two gained scales with Tare values
  float readTaredA();
  float readTaredB();

  // total value
  float readTaredTotal();
  /**
   ...
   * @return gain_
   */
  uint8_t get_gain() { return gain_; }

  /**
   * Set the scale factor
   * @param scale desired scale
   */
  void set_scale(float scale = 1.0f) { scale_ = scale; };
   ...
   */
  void set_offset(int offset = 0) { offset_ = offset; }
...
private:
  ...
  float multA_; // scale A multiplier
  float multB_; // scale B multiplier
